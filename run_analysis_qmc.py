...
- plots (and calculates) how the quasi-monte carlo error depends on the wavenumber k.

- Calculates statistics about the effectiveness of nearby preconditioning for qmc methods for the Helmholtz equation.
...
M - either:
    ...
    an int, where the number of integration points was (for QMC) 2**M, or

    the string 'scale_with_k', in which case M is calculated to keep the QMC error bounded
...
mode - a string, either 'qmc_error' or 'GMRES'. If the former, then
plots are given for qmc errors, if the latter, then GMRES statistics are
given.
...
if mode == 'qmc_error':
    ...
    # Total # solves
    # GMRES as a proportion
    # Av GMRES its escluding direct solves
    # max GMRES its
